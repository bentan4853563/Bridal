...
export default function EditPayment() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (params.id) {
        const paymentData = await handleGetPaymentById(params.id);
        setSelectedCustomer(paymentData.customer);
        setAmount(paymentData.amount);
        setPaymentType(paymentData.paymentType);
        setDate(new Date(paymentData.date).toISOString().split("T")[0]); // Format date for input
        setModality(paymentData.modality);
        setComments(paymentData.comments);
      }
    };
    fetchData();
  }, [params.id]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (params.id) {
      handleUpdatePayment(params.id, paymentData, () => {
        navigate("/payments");
      });
    }
  };
  ...
  return (
    <div className="text-black h-screen flex flex-col overflow-y-auto">
      <div className="bg-white px-4 py-6 border-b flex justify-between items-center">
        <span className="text-2xl">Payment / Edit</span>
      </div>
          ...
          <div className="flex flex-col md:flex-row md:justify-between md:gap-8">
            ...
            <div className="flex-1">
              <label>
                ...
                <input
                  type="date"
                  value={date} // Use the string state directly
                  onChange={(e) => setDate(e.target.value)} // Set the value directly
                  className="border p-2 rounded-md w-full"
                  required
                />
              </label>
